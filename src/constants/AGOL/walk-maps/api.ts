...
export const getPreserveWithBBoxQuery = ({
    ...
    return `${baseURL}${layerId}${typeQuery}${bboxQuery}${preserveBbox}`;
}
...
export const getEndpoints = (orgId: string = import.meta.env.VITE_AGOL_ORG_ID, serviceName: string = import.meta.env.VITE_AGOL_SERVICE_NAME): Endpoints => {
    const baseURL = getBaseURL(orgId, serviceName);
